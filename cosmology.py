...
# Hubble parameter
H = lambda T: T**2/(MPL/np.sqrt(8*np.pi**3.*g_rho(T)/90)) # GeV

# Reduced Hubble parameter \tilde{H}
H_t = lambda T: H(T)/(1+gtilda(T)) # GeV

# Entropy density
s_ent = lambda T: h_s(T)*4*np.pi**2*T**3/90; # GeV^3
...
Y_x_eq_massive = lambda T, m: 45.0/(2*np.pi**4*h_s(T))*(np.pi/8)**(1/2)*(m/T)**(3/2)*np.exp(-m/T) # 1
